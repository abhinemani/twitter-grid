	            ...
	            success: function(data, textStatus, xhr) {
	                ...
	                try {
	                  ...
	                  for (var i = 0; i < JQTWEET.numTweets; i++) {		
						...
						avatar = '<img src="' + data[i].user.profile_image_url + '" class="avatar"  />';
	                    try {
	                      if (data[i].entities['media']) {
	                        img = '<a href="' + url + '" target="_blank"><img class="media" src="' + data[i].entities['media'][0].media_url + '" /></a>';
	                      }
	                    } catch (e) {  
	                      //no media
	                    }
	                    ...
	                    $(JQTWEET.appendTo).append( JQTWEET.template.replace('{TEXT}', data[i].text )
	                        .replace('{USER}', data[i].user.screen_name)
	                        .replace('{IMG}', img)      
	                        .replace('{AVATAR}', avatar)                                                          
	                        .replace('{AGO}', JQTWEET.timeAgo(data[i].created_at) )
	                        .replace('{URL}', url )
						    .replace('{ID}', data[i].id_str )				                            
	                        );
